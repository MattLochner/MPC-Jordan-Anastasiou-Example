...
function MPC_OF(uv, p)
    ...
    uv_ext[1:end-1] .= uv                 #dunno what effect doing something like this would have on the optimisation solver...
    uv_ext[end] = uv[end]
    t_start = p.t[p.i]
    t_horizon = p.t[p.i+p.p.H]
    t = t_start:p.p.t_step:t_horizon
    p.p.fx = make_fx(uv_ext, t)
...
p2 = Params2(10.0, 0.5, 1.0, 5, 0.1, eg_fx, x_est[i,3])

# Create MPC parameter struct for this iteration
display(t)
mpc_p2 = Mpc_p2(p2, xSP, x_est, t, i, uv_ext)

checkJ = MPC_OF(uv_init, mpc_p2)

#@benchmark MPC_OF($uv_init, $mpc_p2)
